...
import requests
import clean
from core import DataFactory, Config
...
class GitHubAPI:
    ...
    def search_user(query, key_type="email"):
        if not GitHubAPI.TOKEN:
            if Config.DATA_DIR == "data/enriched": # Only warn once or if specifically relevant
                 pass 
            return None
...
class Enricher:
    LEGACY_FILE = "data/raw/core_legacy_contributors.parquet"
    OUTPUT_FILE = "data/enriched/core_contributors.parquet"
    ...
    def enrich(commits):
        ...
        manual_locations = {}
        if os.path.exists("metadata/locations.json"):
            with open("metadata/locations.json", "r") as f:
